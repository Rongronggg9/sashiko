...
pub struct OpenAiCompatSettings {
    ...
    /// `AiResponse`. Defaults to `"reasoning_content"`.
...
"openai" | "openai-compatible" => {
    ...
    let max_tokens = ai.openai_compat
        .as_ref()
        .and_then(|c| c.max_tokens)
        .unwrap_or(4096);

    let use_json_schema = ai.openai_compat
        .as_ref()
        .is_some_and(|c| c.use_json_schema);

    let reasoning_field = ai.openai_compat
        .as_ref()
